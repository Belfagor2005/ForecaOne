    ...
    def load_data(self):
        for i, (label, path) in enumerate(self.favs):
            ...
            if current:
                ...
                # Weather icon
                icon_path = join(PLUGIN_PATH, "thumb", f"{current.condition}.png")
                if exists(icon_path):
                    self[f"weather_icon_{i}"].instance.setPixmapFromFile(icon_path)
                else:
                    fallback = join(PLUGIN_PATH, "thumb", "d000.png")
                    self[f"weather_icon_{i}"].instance.setPixmapFromFile(fallback)

                # Wind direction icon
                wind_dir_code = self.fp.degreesToWindDirection(current.wind_direction)
                wind_icon_path = join(PLUGIN_PATH, "thumb", f"{wind_dir_code}.png")
                if exists(wind_icon_path):
                    ...
                    self[f"wind_dir_icon_{i}"].instance.setPixmapFromFile(fallback)
