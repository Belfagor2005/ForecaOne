...
# Directory per la cache delle tile OSM
...
class ForecaMapViewer(Screen, HelpableScreen):
    ...
    def download_tile(self, url, cache_dir, prefix=''):
        cache_key = prefix + url
        cache_file = join(cache_dir, hashlib.md5(cache_key.encode()).hexdigest() + '.png')
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if DEBUG:
                    print(f"[ForecaMapViewer] Tile download error {r.status_code}: {url}")
        except Exception as e:
        ...
        def download_thread():
            ...
            for dx in range(-offset_cols, offset_cols + 1):
                for dy in range(-offset_rows, offset_rows + 1):
                    ...
                    osm_path = self.download_tile(osm_url, OSM_CACHE_DIR, prefix='osm')
                    if osm_path:
    ...
    def merge_tile_grid(self, osm_tiles, foreca_tiles):
        try:
            ...
            out_path = join(CACHE_BASE, f"merged_{self.layer_id}_{self.zoom_level}.jpg")
    ...
    def pan_left(self):
        tile_size_deg = 360.0 / (2 ** self.zoom_level)
        step = tile_size_deg * 0.2
    ...
    def pan_up(self):
        tile_size_deg_lat = 180.0 / (2 ** self.zoom_level)
        step = tile_size_deg_lat * 0.2
