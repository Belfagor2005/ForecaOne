...
class Foreca_Preview(Screen, HelpableScreen):
    ...
    def _moon_api_callback(self, api_data):
        """Callback called by the thread with API data."""
        if api_data:
            from twisted.internet import reactor

            def update_ui():
                if "moonrise_value" in self and api_data["rise"] != "N/A":
                    self["moonrise_value"].setText(api_data["rise"])
                if "moonset_value" in self and api_data["set"] != "N/A":
                    self["moonset_value"].setText(api_data["set"])

                if api_data["phase"] != "N/A" and "moon_label" in self:
                    self["moon_label"].setText(_(api_data["phase"]))
                if api_data["illumination"] is not None and "moon_illum" in self:
                    self["moon_illum"].setText(_("Visibility") + f" {api_data['illumination'] * 100:.1f}%")
